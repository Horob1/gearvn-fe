          ...
          <form method="dialog">
            ...
            <div className="flex justify-center items-center mt-4">
              <p className="inline-flex items-center text-gray-700 font-medium text-xs text-center">
                <span className="ml-2">{!tab?'Chưa có tài khoản?' : 'Đã có tài khoản'}<button type="button" onClick={
                () => setTab(!tab)} className="text-xs ml-2 text-blue-500 font-semibold">{`${!tab?'Đăng ký': 'Đăng nhập'} ➡️`}</button></span>
              </p>
            </div>
          </form>
        </div>
