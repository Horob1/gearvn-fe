...
const SearchPage = () => {
  ...
  const toggleBrand = (brand: string) => {
    if(brands.includes(brand))  setBrands([...brands.filter(item => item!==brand)])
  ...
  return (
    <div className="m-auto xl:max-w-[1220px] lg:max-w-[1000px] md:max-w-[100%] p-4 flex flex-col gap-4 justify-between items-center">
      <Helmet>
        <meta charSet="utf-8" />
        <title>Kết quả tìm kiếm cho ""</title>
        <meta name="description" content="Tìm kiếm sản phẩm của hệ thống GEARVN" />
      ...
      <div className="px-4 pb-12 rounded-md gap-8 flex flex-col py-8 w-full bg-white">
        <div className="flex flex-col gap-4 m-auto ">
          ...
          <form className='relative flex'>
            <input type="text" required placeholder='Bạn cần tìm gì?' className='py-2 px-2 text-base lg:w-[312px] font-medium rounded-md outline-double outline-2 outline-red-400 focus:outline-red-700 pr-12 '/>
        ...
        <div className="w-full">
          ...
          <div className="flex gap-4 items-baseline">
            <span>Kết quả tìm kiếm cho:</span><span className="text-gray-500 text-lg">laptop acer</span><span className="text-red-600 text-lg">(có 90 kết quả)</span>
          </div>
          <div className="p-10 mt-12 flex flex-col gap-4">
            <svg className="m-auto" width="132" height="170" viewBox="0 0 132 170" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div>
              <p className="text-center">Rất tiếc, chúng tôi không tìm thấy kết quả cho từ khóa của bạn</p>
              <p className="text-center">Vui lòng kiểm tra chính tả, sử dụng các từ tổng quát hơn và thử lại!</p>
            </div>
          </div>
          <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 w-full">
            {/* <ProductCard isLoading={false}></ProductCard> */}
            {/* <ProductCard isLoading={true}></ProductCard>
            <ProductCard isLoading={true}></ProductCard>
            <ProductCard isLoading={true}></ProductCard>
            <ProductCard isLoading={true}></ProductCard>
            <ProductCard isLoading={true}></ProductCard>
